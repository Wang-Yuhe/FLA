...
def pattern_com(n=100000,k=317,p=0.05,beta=0.05,project_iter=10,alpha=0.5,comp_iter=1):
    ...
    subsample = random.sample(b.area_by_name["A"].winners, subsample_size)
    b.area_by_name["A"].winners = subsample
    for i in range(comp_iter):
        b.project({},{"A":["A"]})
    return b.area_by_name["A"].saved_w,b.area_by_name["A"].saved_winners
...
def pattern_com_repeated(n=100000,k=317,p=0.05,beta=0.05,project_iter=12,alpha=0.4,
    ...
    for trail in range(trials):
        ...
        while True:
            rounds += 1
            b.project({},{"A":["A"]})
            if (b.area_by_name["A"].num_first_winners == 0) or (rounds == max_recurrent_iter):
                break
        rounds_to_completion.append(rounds)
    saved_winners = b.area_by_name["A"].saved_winners
...
def pattern_com_alphas(n=100000,k=317,p=0.01,beta=0.05,
    ...
    for i in range(project_iter-1):
        b.project({"stim":["A"]},{"A":["A"]})
    results = {}
    A_winners = b.area_by_name["A"].winners
    for alpha in alphas:
        # pick random subset of the neurons to fire
        subsample_size = int(k*alpha)
        b_copy = copy.deepcopy(b)
        subsample = random.sample(b_copy.area_by_name["A"].winners, subsample_size)
        b_copy.area_by_name["A"].winners = subsample
        for i in range(comp_iter):
            b_copy.project({},{"A":["A"]})
        final_winners = b_copy.area_by_name["A"].winners
...
def pattern_com_iterations(n=100000,k=317,p=0.01,beta=0.05,alpha=0.4,comp_iter=8,
    ...
    for i in range(min_iter,max_iter+1):
        ...
        for j in range(comp_iter):
            b_copy.project({},{"A":["A"]})
        o = bu.overlap(b_copy.area_by_name["A"].winners, b.area_by_name["A"].winners)
...
def association_sim(n=100000,k=317,p=0.05,beta=0.1,overlap_iter=10):
    ...
    return b.area_by_name["C"].saved_w,b.area_by_name["C"].saved_winners
...
def association_grand_sim(n=100000,k=317,p=0.01,beta=0.05,min_iter=10,max_iter=20):
    ...
    for i in range(min_iter,max_iter+1):
        ...
        b_copy2.project({},{"B":["C"]})
        o = bu.overlap(b_copy1.area_by_name["C"].winners, b_copy2.area_by_name["C"].winners)
        results[i] = float(o)/float(k)
    return results
...
def merge_sim(n=100000,k=317,p=0.01,beta=0.05,max_t=50):
    ...
    for i in range(max_t-1):
        b.project({"stimA":["A"],"stimB":["B"]},
            {"A":["A","C"],"B":["B","C"],"C":["C","A","B"]})
    return b.area_by_name["A"].saved_w, b.area_by_name["B"].saved_w, b.area_by_name["C"].saved_w
...
def density(n=100000,k=317,p=0.01,beta=0.05,rounds=20):
    ...
    for i in range(rounds):
        b.project({"stim":["A"]},{"A":["A"]})
        saved_w.append(b.area_by_name["A"].w)
    conn = b.connectomes["A"]["A"]
    final_winners = b.area_by_name["A"].winners
    edges = 0
...
def fixed_assembly_recip_proj(n=100000, k=317, p=0.01, beta=0.05):
    ...
    print("Before B->A, A.w=" + str(b.area_by_name["A"].w))
    b.area_by_name["A"].unfix_assembly()
    b.project({},{"B":["A"]})
    print("After B->A, A.w=" + str(b.area_by_name["A"].w))
    for i in range(20):
        b.project({}, {"B":["A"],"A":["A"]})
        print("A.w=" + str(b.area_by_name["A"].w))
    overlaps = bu.get_overlaps(b.area_by_name["A"].saved_winners[-22:],0,percentage=True)
    print(overlaps)
...
def separate(n=10000, k=100, p=0.01, beta=0.05, rounds=10, overlap=0):
    ...
    b.area_by_name["EXP"].winners = list(range(0, k))
    b.area_by_name["EXP"].fix_assembly()
    ...
    for i in range(rounds):
        b.project({}, {"EXP": ["A"], "A": ["A"]})
        new_w = b.area_by_name["A"].w - prev_w
        print(new_w)
        prev_w = b.area_by_name["A"].w
    stim1_assembly = b.area_by_name["A"].winners

    print("PROJECTION STIM_2 INTO A....")


    b.area_by_name["EXP"].winners = list(range(k-overlap, 2*k-overlap))
    b.area_by_name["EXP"].fix_assembly()
    ...
    for i in range(rounds):
        ...
        print(new_w)
        prev_w = b.area_by_name["A"].w
    stim2_assembly = b.area_by_name["A"].winners
    ...
    b.area_by_name["EXP"].fix_assembly()
    b.project({}, {"EXP": ["A"]})
    o = bu.overlap(b.area_by_name["A"].winners, stim1_assembly)
    print("Restored " + str(o) + " / " + str(k) + " of Assembly 1")

    b.area_by_name["EXP"].winners = list(range(k-overlap, 2*k-overlap))
    b.project({}, {"EXP": ["A"]})
    o = bu.overlap(b.area_by_name["A"].winners, stim2_assembly)
